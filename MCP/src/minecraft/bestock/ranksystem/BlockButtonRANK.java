...
public class BlockButtonRANK extends Block
{
    /** Whether this button is sensible to arrows, used by wooden buttons. */
    protected boolean sensible;

    protected BlockButtonRANK(int par1, int par2, boolean par3)
    {
    ...
    {
    	int rank = 0;
    	EntityPlayerMP target = MinecraftServer.getServer().getConfigurationManager().getPlayerForUsername(par5EntityPlayer.username);
    	NBTTagCompound tag = null; if(target != null){tag = target.getEntityData();}

    	NBTBase modeTag = null; if(tag != null) {modeTag = tag.getTag("PlayerRank");}
    	if(modeTag == null)
    	{
    		//player has no rank
    		rank = 0;
    	...
    	else
    	{
    		...
    		{
    		par5EntityPlayer.addChatMessage("You do not have a rank of " + 5 + ", ACCESS DENIED (rank of " + rank + ")");
    		}
    		return true;
    	}
    }
